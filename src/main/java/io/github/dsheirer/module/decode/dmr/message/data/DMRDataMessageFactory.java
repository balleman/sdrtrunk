...
import io.github.dsheirer.module.decode.dmr.message.data.header.UnifiedSingleBlockData;
                                     ...
                                     int timeslot)
    {
        ...
        if(slotType.isValid())
        {
            switch(slotType.getDataType())
            {
                ...
                case UNIFIED_SINGLE_BLOCK_DATA:
                    CorrectedBinaryMessage usbdPayload = getPayload(message);
                    switch(UnifiedSingleBlockData.getServiceType(usbdPayload))
                    {
                        case LIP_SHORT_LOCATION_REQUEST:
                        default:
                            return new UnifiedSingleBlockData(pattern, getPayload(message), cach, slotType, timestamp, timeslot);
                    }
