    ...
    public void receive(Dibit dibit)
    {
        ...
        if(mSynchronized)
        {
            if(mDibitCounter >= BURST_DIBIT_LENGTH)
            {
                ...
                {
                    //We were synchronized but now we're not.  Update the sync detector with the current sync value from
                    //the message so that it can attempt to regain sync.
                    long current = getSyncFieldValue();
                    mSyncDetector.setCurrentSyncValue(getSyncFieldValue());

                    //Trigger a sync detect in case the sync value results from a mis-locked PLL
                    mSyncDetector.hasSync();
                }
        ...
        {
            if(mSyncDetector.hasSync(dibit))
            {
                toggleSyncTracker();
            }
